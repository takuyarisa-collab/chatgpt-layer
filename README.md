...
- 固定エンジン（`chatgpt-layer.user.js` と `chatgpt-layer.meta.js`）は専用チャットで管理する
- Shion用チャットでは、原則として `chatgpt-layer.config.json` 内の `shion` レイヤーと関連ドキュメントだけを更新する
- Shion側の要望に固定エンジン変更が必要な場合、このチャットでは実装せず、必要な変更内容と理由を専用チャットへ渡す
- 別チャットの古い取得内容を使った固定エンジンの全面上書きは禁止する
- やむを得ず固定エンジンへ触れる場合は、直前に最新版を再取得し、TaCの了承を得てから差分を適用する
