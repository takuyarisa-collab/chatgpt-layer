...
(function () {
  ...
  __modules["src/features/composer-background.js"] = (() => {
    ...
    function createComposerBackgroundFeature({
      ...
      function restoreRoot() {
        ...
        if (rootOriginal.attribute === null) {
          root.removeAttribute?.(ROOT_ATTRIBUTE);
        } else {
          root.setAttribute?.(ROOT_ATTRIBUTE, rootOriginal.attribute);
        }
      ...
      function decorateSurface(surface) {
        ...
        surface.setAttribute?.(SURFACE_ATTRIBUTE, "on");
      }
      ...
      function restoreSurface(surface) {
        ...
        if (original === null) {
          surface.removeAttribute?.(SURFACE_ATTRIBUTE);
        } else {
          surface.setAttribute?.(SURFACE_ATTRIBUTE, original);
        }
    ...
    function findComposerSurface(document, getComputedStyle = globalThis.getComputedStyle) {
      ...
      for (let depth = 0; current && depth < MAX_ANCESTOR_DEPTH; depth += 1) {
        if (isComposerSizedSurface(current, editorRect, getComputedStyle)) {
          return current;
        }
    ...
    html[${ROOT_ATTRIBUTE}="on"] div:has(> #prompt-textarea),
    html[${ROOT_ATTRIBUTE}="on"] div:has(> div > #prompt-textarea),
    html[${ROOT_ATTRIBUTE}="on"] [${SURFACE_ATTRIBUTE}="on"] {
  ...
  __modules["src/product-identity.js"] = (() => {
    ...
    const DEVELOPMENT_VERSION = "0.3.0-dev5";
  ...
  __modules["src/update/update-checker.js"] = (() => {
    ...
    function createUpdateChecker({
      ...
      if (typeof fetch !== "function") {
        throw new TypeError("Update checker requires fetch.");
      }
  ...
  __modules["src/userscript/main.js"] = (() => {
    ...
    async function bootstrap() {
      ...
      async function refreshUpdateState({ force = false } = {}) {
        if (!updateChecker) {
          updateState = createUpdateErrorState("fetch_unavailable");
