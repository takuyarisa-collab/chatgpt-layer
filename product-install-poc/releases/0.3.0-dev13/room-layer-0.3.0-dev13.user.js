...
(function () {
  ...
  __modules["src/product-identity.js"] = (() => {
    ...
    const DEVELOPMENT_VERSION = "0.3.0-dev13";
  ...
  __modules["src/userscript/main.js"] = (() => {
    ...
    async function bootstrap() {
      ...
      const stopNavigationWatcher = watchNavigation((reason) => {
        void reconcileCurrentContext({ reason });
      });
    
      const handleVisibilityChange = () => {
        if (document.visibilityState !== "visible") return;
        void reconcileCurrentContext({ force: true, reason: "visibilitychange" });
      ...
      const handlePageShow = (event) => {
        ...
        void reconcileCurrentContext({ force: true, reason: "pageshow" });
        if (tabNavigation.getState() !== "connected") {
          void tabNavigation.ping();
        }
      };
      ...
      async function reconcileCurrentContext({ force = false, reason = "unknown" } = {}) {
        const syncGeneration = ++storageSyncGeneration;
        const nextContext = parseChatGptContext(globalThis.location);
        if (!nextContext.supported) return false;
    
        const contextChanged = !isSameChatGptContext(context, nextContext);
        const reloadSettings = force || shouldReloadSettingsForReconcile(reason);
        if (!contextChanged && !reloadSettings) return false;
        ...
        if (reloadSettings) {
          ...
          nextSettings = nextLoadResult.settings;
          recovered = nextLoadResult.recovered;
        } else if (
          syncGeneration !== storageSyncGeneration ||
          !runtimeGuard.isCurrent() ||
          document.visibilityState === "hidden"
    ...
    function isSameChatGptContext(left, right) {
      return Boolean(
        left &&
        right &&
    ...
    function shouldReloadSettingsForReconcile(reason) {
      return reason === "visibilitychange" || reason === "pageshow";
    }
    
    function watchNavigation(onChange) {
      let previousHref = globalThis.location.href;
    
      const check = (reason = "url-change") => {
        const nextHref = globalThis.location.href;
        const hrefChanged = nextHref !== previousHref;
        if (!hrefChanged && reason !== "popstate") return;
        previousHref = nextHref;
        onChange(reason);
      };
    
      const timer = globalThis.setInterval(() => check("url-change"), NAVIGATION_POLL_MS);
      const handlePopState = () => check("popstate");
