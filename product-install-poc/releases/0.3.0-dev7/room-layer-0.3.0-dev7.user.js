...
(function () {
  ...
  __modules["src/features/wood-desk.js"] = (() => {
    ...
    function createWoodDeskController({ document }) {
      ...
      function apply(composer) {
        ...
        if (composer === activeComposer && isValidStructure(composer)) {
          ensureComposerAttributes(composer);
          return getStructure(composer);
        }
    
        clear();
    
        const existing = getStructure(composer);
        if (existing) {
          activeComposer = composer;
          activeSurface = existing.surface;
      ...
      function clear() {
        ...
        const structure = getStructure(composer) ?? (
          activeSurface
            ? {
                surface: activeSurface,
                mat: composer.parentElement,
                composer
              }
            : null
        );
        ...
        if (
          structure?.surface?.parentNode &&
          structure?.mat &&
          composer.parentNode === structure.mat
        ) {
          structure.surface.parentNode.insertBefore(composer, structure.surface);
          structure.surface.remove?.();
        } else {
          structure?.surface?.remove?.();
        }
    ...
    function ensureComposerAttributes(composer) {
      ...
      composer.style?.setProperty?.("margin-top", "0", "important");
      composer.style?.setProperty?.("margin-right", "0", "important");
      composer.style?.setProperty?.("margin-bottom", "0", "important");
      composer.style?.setProperty?.("margin-left", "0", "important");
      composer.style?.setProperty?.("box-sizing", "border-box", "important");
    }
    
    function isValidStructure(composer) {
      return Boolean(getStructure(composer));
    }
    
    function getStructure(composer) {
      const mat = composer?.parentElement;
      const surface = mat?.parentElement;
      if (
        mat?.getAttribute?.(WOOD_DESK_MAT_ATTRIBUTE) !== "on" ||
        surface?.getAttribute?.(WOOD_DESK_SURFACE_ATTRIBUTE) !== "on"
      ) {
        return null;
      }
      return { surface, mat, composer };
  ...
  __modules["src/features/composer-background.js"] = (() => {
    ...
    function createComposerBackgroundFeature({
      ...
      function decorateRoot(color, appearance) {
        ...
        root.style?.setProperty?.(COLOR_PROPERTY, color, "important");
      }
      ...
      function decorateSurface(surface, appearance) {
        ...
        surface.setAttribute?.(SURFACE_ATTRIBUTE, appearance);
      }
    ...
    function ensureStyle(document) {
      ...
      style.id = STYLE_ID;
      style.textContent = `
    html[${ROOT_ATTRIBUTE}="on"][${APPEARANCE_ATTRIBUTE}="solid"] [class*="bg-token-composer-surface-primary"],
    html[${ROOT_ATTRIBUTE}="on"][${APPEARANCE_ATTRIBUTE}="solid"] form:has(#prompt-textarea),
    html[${ROOT_ATTRIBUTE}="on"][${APPEARANCE_ATTRIBUTE}="solid"] div:has(> #prompt-textarea),
    html[${ROOT_ATTRIBUTE}="on"][${APPEARANCE_ATTRIBUTE}="solid"] div:has(> div > #prompt-textarea),
    html[${ROOT_ATTRIBUTE}="on"] [${SURFACE_ATTRIBUTE}="solid"],
    html[${ROOT_ATTRIBUTE}="on"] [${SURFACE_ATTRIBUTE}="wood"],
    html[${ROOT_ATTRIBUTE}="on"] [${SURFACE_ATTRIBUTE}="wood"] [class*="bg-token-composer-surface-primary"],
    html[${ROOT_ATTRIBUTE}="on"] [${SURFACE_ATTRIBUTE}="wood"] form:has(#prompt-textarea),
    html[${ROOT_ATTRIBUTE}="on"] [${SURFACE_ATTRIBUTE}="wood"] div:has(> #prompt-textarea),
    html[${ROOT_ATTRIBUTE}="on"] [${SURFACE_ATTRIBUTE}="wood"] div:has(> div > #prompt-textarea) {
    ...
    html[${ROOT_ATTRIBUTE}="on"] [${WOOD_DESK_SLOT_ATTRIBUTE}="on"] {
      width: 100% !important;
    ...
    return { createComposerBackgroundFeature, findComposerSurface, findCurrentEditor };
  })();
  ...
  __modules["src/product-identity.js"] = (() => {
    ...
    const DEVELOPMENT_VERSION = "0.3.0-dev7";
  ...
  __modules["src/i18n/messages.js"] = (() => {
    ...
    const MESSAGES = Object.freeze({
      en: Object.freeze({
        ...
        "action.installUpdate": "Update Room Layer",
      ...
      ja: Object.freeze({
        ...
        "action.installUpdate": "Room Layerを更新",
  ...
  __modules["src/userscript/main.js"] = (() => {
    ...
    async function bootstrap() {
      ...
      const panel = createSettingsPanel({
        ...
        onInstallUpdate: () => navigateToUpdate(DEVELOPMENT_DOWNLOAD_URL),
    ...
    function navigateToUpdate(downloadUrl) {
