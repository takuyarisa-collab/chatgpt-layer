...
function normalizeGradient(value, fallbackBackground) {
if (!isRecord(value) || value.enabled === false) return null;
return {
angle: clampNumber(value.angle, 180, -360, 360),
from: normalizeColor(value.from, fallbackBackground),
to: normalizeColor(value.to, fallbackBackground)
};
}
...
return {
color: normalizeColor(value.color, defaults.color || "rgba(0,0,0,0.42)"),
x: clampNumber(value.x, defaults.x || 0, -40, 40),
y: clampNumber(value.y, defaults.y === undefined ? 10 : defaults.y, -40, 40),
blur: clampNumber(value.blur, defaults.blur === undefined ? 24 : defaults.blur, 0, 80),
spread: clampNumber(value.spread, defaults.spread === undefined ? -8 : defaults.spread, -40, 40)
};
}
...
function normalizeVisualStyle(value, fallbackBackground) {
if (!isRecord(value) || value.enabled === false) return null;
return {
background: normalizeColor(value.background, fallbackBackground),
gradient: normalizeGradient(value.gradient, fallbackBackground),
shadow: normalizeShadow(value.shadow),
highlight: normalizeHighlight(value.highlight),
texture: normalizeTexture(value.texture)
};
}
function normalizeComposerStyle(value, composerBackground) {
return normalizeVisualStyle(value, composerBackground);
}
function normalizeDeskSurface(value, composerBackground) {
var visual = normalizeVisualStyle(value, composerBackground);
if (!visual) return null;
visual.padding = clampNumber(value.padding, 6, 2, 18);
visual.radius = clampNumber(value.radius, 28, 12, 52);
return visual;
}
function normalizeDeskMat(value, composerBackground) {
if (!isRecord(value) || value.enabled === false) return null;
return {
background: normalizeColor(value.background, "#5a5360"),
padding: clampNumber(value.padding, 2, 0, 12),
radius: clampNumber(value.radius, 22, 8, 48),
borderSize: clampNumber(value.borderSize, 1, 0, 4),
borderColor: normalizeColor(value.borderColor, "rgba(255,255,255,0.06)"),
shadow: normalizeShadow(value.shadow, {
...
var deskSurface = normalizeDeskSurface(value.deskSurface, composerBackground);
var deskMat = deskSurface ? normalizeDeskMat(value.deskMat, composerBackground) : null;
if (!deskMat) deskSurface = null;
return {
